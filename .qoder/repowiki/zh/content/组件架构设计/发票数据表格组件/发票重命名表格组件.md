...
## 更新摘要
**变更内容**
- 组件已从InvoiceTable.vue重命名为InvoiceRenameTable.vue，同步更新文档中所有相关引用
- 内部日志输出已更新为[InvoiceRenameTable]前缀，与新组件名称保持一致
- 更新了架构图和代码示例以反映最新文件路径
...
InvoiceRenameTable组件是一个专门用于发票重命名功能的结果展示组件，基于Naive UI的n-data-table进行封装。该组件的核心职责是提供发票文件的可视化展示、编辑和管理功能，包括文件名预览、购买方名称编辑、金额修改、新文件名生成等功能。
...
subgraph "核心业务模块"
InvoiceRenameTable[InvoiceRenameTable.vue<br/>发票重命名表格组件]
InvoiceStore[invoice.ts<br/>发票状态管理]
NameGenerator[nameGenerator.ts<br/>文件名生成器]
InvoiceRenameView[InvoiceRename.vue<br/>发票重命名视图]
end
Views --> InvoiceRenameView
Components --> InvoiceRenameTable
Stores --> InvoiceStore
Utils --> NameGenerator
InvoiceRenameView --> InvoiceRenameTable
InvoiceRenameTable --> InvoiceStore
InvoiceStore --> NameGenerator
```

**图表来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L1-L182)
...
InvoiceRenameTable组件是整个发票重命名功能的核心，具有以下关键特性：

### 组件架构设计

```mermaid
classDiagram
class InvoiceRenameTable {
...
InvoiceRenameTable --> InvoiceRow : "渲染数据"
InvoiceRenameTable --> InvoiceStore : "事件通信"
InvoiceRenameView --> InvoiceRenameTable : "父组件"
```

**图表来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L19-L39)
...
InvoiceRenameTable组件在整个应用架构中扮演着承上启下的关键角色，连接了用户界面与业务逻辑：

```mermaid
sequenceDiagram
participant User as 用户
participant InvoiceRenameView as 发票重命名视图
participant InvoiceRenameTable as 发票重命名表格
participant Store as 发票状态管理
participant NameGen as 文件名生成器
User->>InvoiceRenameView : 上传发票文件
InvoiceRenameView->>Store : addFile(file, result)
Store->>NameGen : generateFileName(data, ext)
NameGen-->>Store : 新文件名
Store-->>InvoiceRenameView : 更新状态
InvoiceRenameView->>InvoiceRenameTable : 传入data和selectedIds
User->>InvoiceRenameTable : 编辑购买方名称/金额
InvoiceRenameTable->>InvoiceRenameView : emit('edit', id, updates)
InvoiceRenameView->>Store : updateFile(id, updates)
Store->>NameGen : 重新生成文件名
NameGen-->>Store : 更新后的文件名
Store-->>InvoiceRenameTable : 状态更新
User->>InvoiceRenameTable : 点击删除按钮
InvoiceRenameTable->>InvoiceRenameView : emit('delete', id)
InvoiceRenameView->>Store : removeFile(id)
```

**图表来源**
- [InvoiceRename.vue](file://src/views/InvoiceRename.vue#L160-L178)
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L35-L39)
...
InvoiceRenameTable组件定义了完整的列配置，每个列都有特定的功能和渲染逻辑：
...
**章节来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L32-L36)
...
**章节来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L100-L117)
...
**章节来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L204-L254)
...
**图表来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L76-L115)
- [InvoiceRename.vue](file://src/views/InvoiceRename.vue#L160-L162)
...
**图表来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L121-L129)
...
**章节来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L41-L162)
...
```mermaid
sequenceDiagram
participant Child as InvoiceRenameTable
participant Parent as InvoiceRenameView
...
**章节来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L30-L39)
- [InvoiceRename.vue](file://src/views/InvoiceRename.vue#L160-L178)
...
**图表来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L6-L8)
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L48-L162)

**章节来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L164-L176)
...
subgraph "内部模块"
InvoiceRenameTable[InvoiceRenameTable.vue]
InvoiceRenameView[InvoiceRename.vue]
InvoiceStore[invoice.ts]
NameGenerator[nameGenerator.ts]
OperationBar[OperationBar.vue]
end
NaiveUI --> InvoiceRenameTable
Vue --> InvoiceRenameTable
Pinia --> InvoiceStore
InvoiceRenameView --> InvoiceRenameTable
InvoiceRenameView --> OperationBar
InvoiceRenameTable --> InvoiceStore
InvoiceStore --> NameGenerator
...
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L13-L15)
...
Store --> FilteredList
Store --> SelectedIds
FilteredList --> InvoiceRenameView
SelectedIds --> InvoiceRenameView
InvoiceRenameView --> InvoiceRenameTable
InvoiceRenameView --> OperationBar
InvoiceRenameTable --> Store
OperationBar --> Store
```

**图表来源**
- [InvoiceRename.vue](file://src/views/InvoiceRename.vue#L26-L32)
...
**章节来源**
- [InvoiceRenameTable.vue](file://src/components/Invoice/InvoiceRenameTable.vue#L121-L129)
- [InvoiceRename.vue](file://src/views/InvoiceRename.vue#L167-L178)
...
InvoiceRenameTable组件作为发票重命名功能的核心组件，展现了现代前端开发的最佳实践：
