...
- 路由层：router/index.ts 定义路由表，包含首页、发票重命名、发票解析与 404。
- 视图层：views 下的 HomePage、Home、InvoiceParsing 对应不同业务页面。
- 布局与通用组件：components/AppLayout.vue 提供侧边菜单、面包屑、标题与内容区插槽；components/NotFound.vue 提供 404 页面与倒计时返回首页。
- 工具与组合式函数：composables/useTheme.ts 等用于主题切换；stores/ 与 composables/ 内部模块负责状态与业务逻辑。

```mermaid
graph TB
A["main.ts<br/>创建应用并注册插件"] --> B["App.vue<br/>全局主题与通知包装器"]
B --> C["AppLayout.vue<br/>布局容器菜单/面包屑/内容插槽"]
C --> D["router-view<br/>路由视图占位"]
E["router/index.ts<br/>路由表定义"] --> D
F["HomePage.vue"] --> D
G["Home.vue"] --> D
...
## 核心组件
- 路由表与历史模式：使用 createRouter 与 createWebHistory，routes 数组定义了三条静态路由与一条通配符 404 路由。
...
- [src/components/AppLayout.vue](file://src/components/AppLayout.vue#L1-L104)
- [src/components/NotFound.vue](file://src/components/NotFound.vue#L1-L20)
...
- [src/components/AppLayout.vue](file://src/components/AppLayout.vue#L135-L249)
...
- [src/components/NotFound.vue](file://src/components/NotFound.vue#L31-L45)
...
- 路由表定义
  ...
  - “/invoice-rename” 映射到 Home（发票重命名），meta.title 用于面包屑与标题。
...
```mermaid
flowchart TD
Start(["进入路由配置"]) --> DefineRoutes["定义 routes 数组<br/>包含首页/重命名/解析/404"]
DefineRoutes --> History["创建 router<br/>history: createWebHistory(BASE_URL)"]
History --> Export["导出路由实例"]
Export --> UseInApp["main.ts 注册路由插件"]
UseInApp --> RenderLayout["App.vue 渲染 AppLayout"]
RenderLayout --> LayoutContainer["AppLayout.vue 渲染 router-view"]
LayoutContainer --> MatchRoute{"匹配当前路径"}
MatchRoute --> |"/"| HomePage["渲染 HomePage.vue"]
MatchRoute --> |"/invoice-rename"| InvoiceRename["渲染 Home.vue"]
MatchRoute --> |"/invoice-parsing"| InvoiceParsing["渲染 InvoiceParsing.vue"]
MatchRoute --> |其他| NotFound["渲染 NotFound.vue"]
```
...
- [src/App.vue](file://src/App.vue#L7-L13)
- [src/components/AppLayout.vue](file://src/components/AppLayout.vue#L88-L92)
- [src/views/HomePage.vue](file://src/views/HomePage.vue#L80-L90)
- [src/views/Home.vue](file://src/views/Home.vue#L1-L247)
- [src/views/InvoiceParsing.vue](file://src/views/InvoiceParsing.vue#L1-L328)
- [src/components/NotFound.vue](file://src/components/NotFound.vue#L1-L20)
...
- [src/router/index.ts](file://src/router/index.ts#L7-L39)
- [src/views/HomePage.vue](file://src/views/HomePage.vue#L80-L90)
- [src/components/AppLayout.vue](file://src/components/AppLayout.vue#L229-L245)

### AppLayout 布局容器与导航协调
- 菜单与面包屑
  - menuOptions 定义两级菜单：首页与“发票工具”分组，分组内包含“发票重命名”“发票解析”两个子项。
...
AppLayout --> Router : "使用 router.push 导航"
```
...
章节来源
- [src/components/AppLayout.vue](file://src/components/AppLayout.vue#L140-L223)

### 视图组件与编程式导航
- HomePage.vue
  - 通过 useRouter 获取 router 实例，在工具卡片点击时调用 router.push 将用户引导至对应功能页面。
- Home.vue 与 InvoiceParsing.vue
...
participant V as "router-view"
U->>HP : 点击工具卡片
HP->>R : router.push("/invoice-rename" 或 "/invoice-parsing")
R-->>V : 切换到目标视图
V-->>L : AppLayout 渲染对应页面
```
...
C --> D["渲染 NotFound.vue"]
D --> E{"倒计时结束?"}
E --> |是| F["router.push('/') 自动返回首页"]
E --> |否| G["等待用户点击按钮"]
G --> H["router.push('/') 手动返回首页"]
```

图表来源
- [src/router/index.ts](file://src/router/index.ts#L26-L31)
- [src/components/NotFound.vue](file://src/components/NotFound.vue#L31-L55)
...
- 路由与视图
  - router/index.ts 定义路由与组件映射；AppLayout 通过插槽接收 router-view；HomePage/Home/InvoiceParsing/NotFound 分别渲染对应页面。
- 导航协作
...
章节来源
- [src/router/index.ts](file://src/router/index.ts#L7-L39)
- [src/components/AppLayout.vue](file://src/components/AppLayout.vue#L140-L147)
- [src/App.vue](file://src/App.vue#L7-L13)
- [src/main.ts](file://src/main.ts#L9-L11)

## 结论
本项目采用简洁清晰的路由与布局设计：router/index.ts 定义三条业务路由与 404 兜底；AppLayout.vue 通过菜单与面包屑统一导航；HomePage 与功能页面通过编程式导航协同工作。整体结构易于维护与扩展，适合进一步引入权限校验、路由懒加载与国际化标题等特性。
...
章节来源
- [src/router/index.ts](file://src/router/index.ts#L7-L39)
- [src/components/AppLayout.vue](file://src/components/AppLayout.vue#L149-L170)
- [src/App.vue](file://src/App.vue#L7-L13)