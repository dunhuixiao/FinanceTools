...
## 简介
本文件面向“发票重命名”功能的文件名生成系统，聚焦于 nameGenerator.js 模块的设计与实现，解释其如何依据发票解析结果中的关键字段（如购买方名称、价税合计等）按预设规则生成新文件名；说明字段格式化策略（金额数字处理、日期格式化、特殊字符清理、长度限制等），以及文件名冲突的处理机制（序号追加）。同时阐述该模块与状态管理 Store 的集成方式，使用户在编辑发票信息后能动态更新“新文件名”，并为开发者提供扩展命名规则的接口与最佳实践。

## 项目结构
围绕文件名生成系统的关键文件分布如下：
- utils/nameGenerator.js：文件名生成与冲突处理的核心实现
- stores/invoice.js：状态管理，负责维护发票条目、触发文件名生成与更新
- views/Home.vue：上传与处理流程入口，调用解析与导出能力
- components/InvoiceTable.vue：表格展示与编辑交互，触发 Store 更新
- composables/useFileParser.js：文件解析组合式函数，产出解析结果
- composables/useFileExport.js：导出 ZIP 组合式函数，使用新文件名进行打包
- utils/xmlParser.js、utils/pdfParser.js：发票数据提取与校验
...
subgraph "状态管理层"
Store["stores/invoice.js<br/>状态与动作"]
end
subgraph "工具层"
Parser["useFileParser.js<br/>解析PDF/XML"]
Export["useFileExport.js<br/>导出ZIP"]
NameGen["utils/nameGenerator.js<br/>生成/冲突处理"]
XML["utils/xmlParser.js<br/>XML提取/校验"]
PDF["utils/pdfParser.js<br/>PDF提取/校验"]
end
Home --> Parser
Home --> Store
Home --> Export
...
- 解析与导出（composables/useFileParser.js、useFileExport.js）
...
participant G as "utils/nameGenerator.js"
...
### 解析与导出（useFileParser.js、useFileExport.js）
- 解析
...
## 依赖分析
- 模块耦合
  - nameGenerator.js 与 invoice.js：通过 Store 的 addFile 与 updateFile 调用生成器，形成稳定的接口契约
  - Home.vue 与 InvoiceTable.vue：通过事件与 Store 协作，驱动状态变更
  - useFileParser.js 与解析器：为 Store 提供解析数据，间接影响文件名生成
  - useFileExport.js：消费 Store 中的状态，决定使用新文件名还是原文件名
- 外部依赖
  - PDF.js：用于 PDF 文本提取
...
```mermaid
graph LR
NameGen["nameGenerator.js"] --> Store["stores/invoice.js"]
Store --> Home["Home.vue"]
...
- 并行解析
  - useFileParser.js 已采用分批并行处理，减少 UI 阻塞，提升吞吐
- 导出阶段
...
- 生成失败：字段缺失或为空
  ...
  - 参考路径
    - [nameGenerator.js](file://src/utils/nameGenerator.js#L125-L145)
- 规则不存在
  - 现象：抛出“命名规则不存在”错误
  ...
  - 参考路径
    ...
    - [nameGenerator.js](file://src/utils/nameGenerator.js#L202-L211)
- 冲突导致文件名异常
  - 现象：生成的新文件名被追加序号
  - 排查：确认是否存在同名文件；检查扩展名是否正确分离
  - 参考路径
    - [nameGenerator.js](file://src/utils/nameGenerator.js#L147-L166)
- 用户编辑后未更新
...
- 导出文件名不符合预期
  ...
  - 参考路径
    - [useFileExport.js](file://src/composables/useFileExport.js#L21-L33)
    - [invoice.js](file://src/stores/invoice.js#L19-L39)

章节来源
- [nameGenerator.js](file://src/utils/nameGenerator.js#L118-L166)
- [InvoiceTable.vue](file://src/components/InvoiceTable.vue#L64-L103)
- [Home.vue](file://src/views/Home.vue#L158-L163)
- [invoice.js](file://src/stores/invoice.js#L101-L124)
- [useFileExport.js](file://src/composables/useFileExport.js#L21-L33)
...
## 结论
文件名生成系统通过清晰的规则定义、稳健的字段格式化与冲突处理机制，实现了对发票文件的自动化重命名。它与状态管理 Store 紧密集成，使得用户在编辑关键字段后能够即时获得新的文件名，从而提升工作流效率与一致性。开发者可通过扩展规则与格式化器轻松适配更多命名需求，并利用现有接口进行测试与灰度发布。
...
章节来源
- [nameGenerator.js](file://src/utils/nameGenerator.js#L9-L35)
- [nameGenerator.js](file://src/utils/nameGenerator.js#L40-L113)
...
章节来源
- [nameGenerator.js](file://src/utils/nameGenerator.js#L9-L35)
- [nameGenerator.js](file://src/utils/nameGenerator.js#L147-L200)
- [nameGenerator.js](file://src/utils/nameGenerator.js#L202-L225)