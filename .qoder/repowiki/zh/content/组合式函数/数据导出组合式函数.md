...
## 更新摘要
**已做更改**
- 更新了`InvoiceJSONFormat`接口定义，移除了`status`和`errorMessage`字段
- 修正了`transformDataForJSON`函数的文档说明，明确不再包含状态信息
- 更新了JSON导出函数的数据转换策略说明
- 移除了与`status`和`errorMessage`字段相关的表格和说明
...
本文档系统化地文档化了useDataExport组合式函数的数据导出能力，该函数提供了Excel和JSON两种格式的发票解析结果导出功能。该组合式函数采用Vue 3 Composition API设计，支持异步导出操作，具有完善的错误处理机制和性能监控功能。根据最新代码变更，文档已更新以反映`status`和`errorMessage`字段从导出数据结构中移除的变更。
...
RemoveFields --> StatusFields["status和errorMessage字段被移除"]
```
...
**图表来源**
- [InvoiceParsing.vue](file://src/views/InvoiceParsing.vue#L273-L302)
- [InvoiceParsingTable.vue](file://src/components/Invoice/InvoiceParsingTable.vue#L82-L98)
