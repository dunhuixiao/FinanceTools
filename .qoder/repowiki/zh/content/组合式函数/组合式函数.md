...
5. [useInvoiceContentParser：发票内容解析](#useinvoicecontentparser发票内容解析)
6. [useDataExport：数据导出功能](#usedataexport数据导出功能)
7. [useFileExport：文件导出功能](#usefileexport文件导出功能)
8. [useTheme：主题管理](#usetheme主题管理)
9. [组合式函数集成与使用模式](#组合式函数集成与使用模式)
10. [扩展性与测试友好性](#扩展性与测试友好性)
11. [结论](#结论)

## 简介
本项目中的组合式函数（Composables）是实现逻辑复用和代码组织的核心机制。这些函数封装了文件解析、数据导出和主题管理等关键业务逻辑，通过Vue 3的Composition API提供响应式状态和方法。`useFileParser`、`useInvoicePdfParser`和`useInvoiceContentParser`分别处理通用文件解析、发票PDF专用解析和发票内容解析，而`useDataExport`和`useFileExport`则封装了不同类型的数据导出逻辑。`useTheme`负责管理应用的主题切换，确保用户体验的一致性。这些组合式函数通过暴露响应式变量、方法和生命周期钩子，实现了高内聚、低耦合的设计，便于在不同组件中复用。

## 核心组合式函数分析
本项目中的组合式函数设计遵循单一职责原则，每个函数专注于特定的业务领域。`useFileParser`提供通用文件解析接口，`useInvoicePdfParser`和`useInvoiceContentParser`分别针对发票PDF的不同层面进行解析，前者关注发票整体信息，后者专注于项目明细行。`useDataExport`和`useFileExport`分别处理结构化数据和原始文件的导出。`useTheme`则独立管理应用的视觉主题。这些函数通过返回包含响应式状态和方法的对象，实现了逻辑的封装和复用。它们与Pinia状态管理store紧密结合，通过调用store的方法来更新应用状态，形成了清晰的数据流。
...
## useInvoiceContentParser：发票内容解析
`useInvoiceContentParser`组合式函数专门用于解析发票PDF中的项目明细行内容，提供了精确的表格识别和数据提取能力。该函数暴露了`isProcessing`、`currentFile`和`progress`三个响应式状态变量，以及`parseContent`和`parseContentBatch`两个核心方法。该函数实现了智能的文件验证机制，支持PDF格式检查和10MB大小限制。`parseContent`方法提供单文件解析功能，而`parseContentBatch`方法实现了分批并发处理，每批处理10个文件，支持进度回调和实时状态更新。

函数内部使用`extractInvoiceContentItems`工具函数进行深度内容提取，该工具采用ETL流水线架构：提取层（Extraction）复用发票PDF解析器的文本提取能力，表格识别层（Table Detection）通过坐标分析和关键词匹配识别表格区域，行提取层（Row Extraction）基于项目起始标记和边界检测提取明细行，最后通过格式校验确保数据质量。

```mermaid
flowchart TD
Start([开始内容解析]) --> ValidateFile["验证文件格式和大小"]
ValidateFile --> ExtractText["提取带坐标文本"]
ExtractText --> DetectTable["检测表格区域和列映射"]
DetectTable --> ExtractRows["提取项目明细行"]
ExtractRows --> ParseRow["解析单行数据"]
ParseRow --> ValidateData["格式校验和过滤"]
ValidateData --> CollectResult["收集解析结果"]
CollectResult --> BatchProcess["批量处理循环"]
BatchProcess --> UpdateProgress["更新进度状态"]
UpdateProgress --> End([结束])
```

**Diagram sources**
- [useInvoiceContentParser.ts](file://src/composables/useInvoiceContentParser.ts#L51-L98)
...
## 组合式函数集成与使用模式
组合式函数在组件中的使用遵循标准模式。在`InvoiceParsing.vue`中，通过`import { useInvoicePdfParser } from '../composables/useInvoicePdfParser'`导入函数，然后在`setup`函数中调用`const { parseBatch, currentFile } = useInvoicePdfParser()`获取响应式状态和方法。在`InvoiceContent.vue`中，通过`import { useInvoiceContentParser } from '../composables/useInvoiceContentParser'`导入发票内容解析函数，并结合`useInvoiceContentStore`和`useContentExport`实现完整的发票内容解析和导出流程。这些状态和方法可以直接在模板中使用，或作为事件处理器的参数。与Pinia store的集成通过`useInvoiceContentStore`实现，解析结果通过`store.addItems(items)`更新到全局状态。这种模式确保了逻辑的清晰分离和高效复用。
...
## 扩展性与测试友好性
组合式函数的设计具有良好的扩展性和测试友好性。每个函数的职责单一，接口清晰，便于单元测试。例如，`useFileParser`的`parseFile`方法可以独立测试不同文件格式的解析逻辑，`useInvoiceContentParser`的`parseContent`方法可以单独测试单个文件的内容解析。函数的依赖通过模块导入，可以使用mock对象进行隔离测试。扩展新功能时，可以创建新的组合式函数或在现有函数中添加新方法，而不会影响现有代码。例如，要支持新的文件格式，可以在`useFileParser`中添加相应的解析逻辑，而无需修改调用方代码。
...
## 结论
本项目中的组合式函数通过Vue 3的Composition API实现了逻辑的高效复用和代码的清晰组织。`useFileParser`提供通用文件解析接口，`useInvoicePdfParser`专注于发票整体信息提取，`useInvoiceContentParser`深入解析项目明细行，`useDataExport`和`useFileExport`封装了不同类型的数据导出逻辑，`useTheme`则统一管理应用的视觉主题。这些函数通过暴露响应式变量和方法，与组件和状态管理store无缝集成，形成了清晰的数据流和职责划分。其设计具有良好的扩展性和测试友好性，为应用的长期维护和发展奠定了坚实的基础。