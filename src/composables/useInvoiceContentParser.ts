 ...
 */
import { ref } from 'vue'
import { extractInvoiceContentItems } from '../utils/invoiceContentParser'
 ...
 * 发票内容解析Composable
 */
export function useInvoiceContentParser() {
  ...
  function validateFile(file: File): { valid: boolean; error?: string } {
    ...
    if (!file.type.includes('pdf') && !file.name.toLowerCase().endsWith('.pdf')) {
      return { valid: false, error: '仅支持PDF格式文件' }
    }
  ...
  async function parseContent(file: File): Promise<InvoiceContentParseResult> {
    ...
    try {
      const items = await extractInvoiceContentItems(file, fileId)
      
      if (isDev) {
        const endTime = performance.now()
        console.log(`[性能] 解析 ${file.name}: ${items.length}行, 耗时: ${((endTime - startTime) / 1000).toFixed(2)}s`)
      }
      
      return {
        fileId,
        fileName: file.name,
        invoiceNumber: items[0]?.sourceInvoiceNumber,
        invoiceDate: items[0]?.sourceInvoiceDate,
        items,
        status: items.length > 0 ? 'success' : 'failed',
        errorMessage: items.length === 0 ? '未解析到项目明细' : undefined,
        parseTime: new Date().toISOString()
      }
    } catch (error) {
      console.error('[内容解析] 文件解析失败:', file.name, (error as Error).message)
  ...
  async function parseContentBatch(
    ...
    if (isDev) {
      console.log(`[批量解析] 开始处理 ${total} 个文件`)
    }
    ...
    try {
      ...
      if (isDev) {
        ...
        console.log(`[性能] 批量解析完成: ${total}文件, ${allItems.length}行, 成功${successCount}个, 耗时: ${((totalEndTime - totalStartTime) / 1000).toFixed(2)}s`)
      }
