<template>
  <div class="page-container">
    ...
    <n-space vertical size="large">
      <n-card title="上传XML发票文件" size="small">
        <file-uploader-batch
          accept="application/xml,.xml,text/xml"
          :disabled="store.isProcessing"
          description="支持 XML 格式的发票文件，单个文件不超过 10MB，最多 100 个文件"
          @upload="handleUpload"
        />
      </n-card>
...
import { useInvoiceContentXMLStore } from '../stores/invoiceContentXML'
import { useInvoiceContentXMLParser } from '../composables/useInvoiceContentXMLParser'
import { useContentExport } from '../composables/useDataExport'

const message = useMessage()
const dialog = useDialog()

const store = useInvoiceContentXMLStore()
const { parseContentBatch } = useInvoiceContentXMLParser()
...
async function handleUpload(files: File[]) {
  ...
  const invalidFiles = files.filter((file: File) => {
    const isXML: boolean = file.name.toLowerCase().endsWith('.xml') || 
                           file.type === 'application/xml' || 
                           file.type === 'text/xml'
    const isSizeValid: boolean = file.size <= 10 * 1024 * 1024
    return !isXML || !isSizeValid
  })
...
async function handleExportSelect(key: string) {
  ...
  try {
    const result = await exportInvoiceContent(dataToExport, '发票内容明细(XML)', isFullExport)
